...
class AdminController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware(function ($request, $next) {
            if (!Auth::user() || Auth::user()->role_id !== RoleType::ADMIN) {
    ...
    public function dashboard()
    {
        ...
        $totalAdmins = RsUser::where('role_id', RoleType::ADMIN)->count();
        $totalBrokers = RsUser::where('role_id', RoleType::BROKER)->count();
        $totalOwners = RsUser::where('role_id', RoleType::OWNER)->count();
        $totalCustomers = RsUser::where('role_id', RoleType::CUSTOMER)->count();
        ...
        $lastMonthRevenue = RsReservation::where('active', true)
            ->whereRaw("strftime('%m', created_at) = ?", [date('m', strtotime('-1 month'))])
            ->whereRaw("strftime('%Y', created_at) = ?", [date('Y', strtotime('-1 month'))])
            ->sum('total_price');
    ...
    public function properties(Request $request)
    {
        ...
        $owners = RsUser::whereIn('role_id', [RoleType::OWNER, RoleType::BROKER])->get();
    ...
    {
        $owners = RsUser::whereIn('role_id', [RoleType::OWNER, RoleType::BROKER, RoleType::ADMIN])->get();
